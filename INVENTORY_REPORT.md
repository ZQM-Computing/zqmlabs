...
| `zqmlabs-gamification` | **REDUNDANT DUPLICATE** | 5 | 38 KB | Python | Gamification engine — duplicate of zqmlabs-portal/backend/gamification.py; archiving recommended |
...
1. **Archive `zqmlabs-gamification`** — redundant duplicate of `zqmlabs-portal/backend/gamification.py`; use `PATCH archived=true`
2. **Remove stale placeholder JSON files** from `zqmlabs-portal/data/` — 5 files (`environment.json`, `government.json`, `safety.json`, `education.json`, `health.json`) — requires PR (branch protection)
3. **Update `zqmlabs-labs/Data/DATA_CATALOG.md`** — update category names from old 10 to new 11 (archived repo, read-only)

### Low Priority
4. **Verify `zqm-web` and `zqm-portal-builder`** — check if they overlap with `zqmlabs-portal/src/`
5. **Verify `zqm-ai-council` and `zqm-ai-master`** — check for overlap
6. **Update `zqmlabs-labs` description** to be more descriptive
7. **Check `zqmlabs-portal` GitHub for `.git.broken` directory** — potential broken git reference
8. **Fix backend health** — `db_exists: false`; database file at `dist/data/volusia.db` is 0 bytes; need to initialize via `scripts/deploy.py`
